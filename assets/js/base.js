// assets/js/base.js v15 - Alliance Hub utilities
// FIX v15: supabase → window.supabase en todas las funciones async
//          generatePlayerToken → window.generatePlayerToken (strict mode compat)
// NOTA: Funciones de autenticacion viven en auth-core.js (single source of truth).
...
window.__AH_BASE_PATH = (function() {
    ...
    if (parts.length >= 1) {
        var knownPages = ['index.html','login.html','login-player.html','rankings.html','game.html','player.html','dashboard.html','reset-password.html','chat.html','404.html','manifest.json','assets','register','service-worker.js','course','landing.html','leader-dashboard.html','alliance-panel.html','alliance.html','rules.html','apply-leader.html','report.html','aviso-legal.html'];
...
function isPlayerBanned(player) {
    return window.AHSanctions ? window.AHSanctions.isPlayerSanctioned(player) : false;
}

function getBanRemainingText(player) {
    if (!window.AHSanctions || !player) return '';
    var summary = window.AHSanctions.getSanctionSummary(player);
...
async function savePlayerSession(playerId, displayName) {
    var result = await window.generatePlayerToken(playerId, displayName);
    if (result && result.success) {
        setPlayerData(playerId, displayName, result.token);
        return true;
    }
    console.error('savePlayerSession failed:', result ? result.message : 'no result');
    return false;
}
