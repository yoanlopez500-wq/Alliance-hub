    ...
    async function loadAlliances() {
        try {
            ...
            c.innerHTML = '<div class="grid grid-cols-1 md:grid-cols-2 gap-4">' + sorted.map(function(a, i) {
                return '<a href="alliance.html?id=' + a.id + '" class="block rounded-xl p-5 bg-slate-900 border border-indigo-900 hover:border-ah-accent transition-colors"><div class="flex items-center gap-3"><div class="text-2xl font-bold text-amber-400">#' + (i+1) + '</div><div class="flex-1"><h3 class="font-bold text-lg">' + escapeHtml(a.name) + '</h3><p class="text-xs text-ah-muted">[' + escapeHtml(a.tag || '-') + ']</p></div></div><div class="grid grid-cols-2 gap-3 mt-3"><div class="rounded-lg p-2 text-center bg-slate-950"><p class="text-xs text-ah-muted">Miembros</p><p class="font-bold">' + (a.member_count || 0) + '</p></div><div class="rounded-lg p-2 text-center bg-slate-950"><p class="text-xs text-ah-muted">Bajas</p><p class="font-bold text-green-500">' + (a.total_kills || 0).toLocaleString() + '</p></div></div></a>';
            }).join('') + '</div>';
