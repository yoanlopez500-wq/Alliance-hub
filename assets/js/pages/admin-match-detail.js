...
if(window.AHRankingScore&&window.AHRankingScore.compareMatchResults){res.sort(window.AHRankingScore.compareMatchResults(function(rw){return (pm[rw.player_id]||{}).current_username||'';}));}
var showActions=canEditResults();var html='<table class="w-full text-sm min-w-full"><thead><tr class="bg-slate-950"><th class="text-left p-3">Jugador</th><th class="text-right p-3">Bajas</th><th class="text-right p-3">Muertes</th><th class="text-right p-3">KD</th><th class="text-center p-3">Valido</th>'+(showActions?'<th class="text-center p-3">Acciones</th>':'')+'</tr></thead><tbody>'+res.map(function(r){var p=pm[r.player_id]||{};var isValid=!!regIds[r.player_id];var validBadge=isValid?'<span class="text-[10px] px-1.5 py-0.5 rounded bg-green-500/15 text-green-500">Si</span>':'<span class="text-[10px] px-1.5 py-0.5 rounded bg-slate-500/15 text-slate-400" title="No registrado en la partida: no cuenta para ranking">No</span>';var actionButtons=showActions?'<td class="p-2 text-center"><button onclick="editResult(\''+r.id+'\')" class="text-xs px-1 py-0.5 rounded mr-1 bg-blue-500/15 text-blue-500" title="Editar">&#9998;</button><button onclick="deleteResult(\''+r.id+'\','+r.player_id+')" class="text-xs px-1 py-0.5 rounded bg-red-500/15 text-red-400" title="Eliminar">&#128465;</button></td>':'';return'<tr class="border-b border-indigo-900"><td class="p-3 font-medium">'+(p.current_username||'Jugador '+r.player_id)+'</td><td class="p-3 text-right text-green-500">'+(r.kills||0)+'</td><td class="p-3 text-right text-red-400">'+(r.deaths||0)+'</td><td class="p-3 text-right font-bold">'+(r.kd_ratio||0)+'</td><td class="p-3 text-center">'+validBadge+'</td>'+actionButtons+'</tr>';}).join('')+'</tbody></table>';document.getElementById('results-list').innerHTML=html;}catch(e){document.getElementById('results-list').innerHTML='<div class="text-center py-4 text-red-400 text-sm">Error: '+e.message+'</div>';}}
