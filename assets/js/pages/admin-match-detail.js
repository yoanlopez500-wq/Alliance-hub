...
document.getElementById('create-match-form').addEventListener('submit',async function(e){e.preventDefault();var name=document.getElementById('cm-name').value.trim();var gameId=document.getElementById('cm-game-id').value.trim()||null;var password=document.getElementById('cm-password').value.trim()||null;var allianceId=document.getElementById('cm-alliance').value||null;var type=document.getElementById('cm-type').value;var category=(document.getElementById('cm-category')||{}).value||'alliance_hub';var whatsappEl=document.getElementById('cm-whatsapp');var whatsappUrl=whatsappEl?(whatsappEl.value.trim()||null):null;var maxPlayers=parseInt(document.getElementById('cm-max').value)||null;var desc=document.getElementById('cm-desc').value.trim()||null;if(!name){window.showToast('Nombre obligatorio','warning');return;}try{var{data:{session}}=await window.supabase.auth.getSession();var{data,error}=await window.supabase.from('matches').insert({name,game_id:gameId,password:password,alliance_id:allianceId,match_type:type,category:category,whatsapp_url:whatsappUrl,max_players:maxPlayers,description:desc,status:'draft',created_by:session.user.id}).select().single();if(error)throw error;window.showToast('Partida creada','success');setTimeout(function(){window.location.href='match-detail.html?id='+data.id;},800);}catch(e){window.showToast('Error: '+e.message,'error');}});

function getSanctionBadgeAndText(player){
    if(!window.AHSanctions || !player)return {badge:'',text:''};
    var s=window.AHSanctions.getSanctionSummary(player);
    if(!s.isSanctioned)return {badge:'',text:''};
    var color=s.type==='banned'?'bg-red-500/15 text-red-500':'bg-amber-500/15 text-amber-400';
    var label=s.type==='banned'?'BANEADO':'SUSPENDIDO';
    return {
        badge:'<span class="text-[10px] px-1.5 py-0.5 rounded ml-2 '+color+'">'+label+'</span>',
        text:s.reason+' · Restante: '+s.remainingText
    };
}
...
async function loadResults(){try{var{data:results,error}=await window.supabase.from('match_results').select('*').eq('match_id',matchId).order('imported_at',{ascending:false});if(error)throw error;var res=results||[];if(res.length===0){lastResultsState=null;document.getElementById('results-list').innerHTML='<div class="text-center py-6 text-sm text-slate-400">Sin resultados</div>';return;}var playerIds=res.map(function(r){return r.player_id;}).filter(function(v,i,a){return a.indexOf(v)===i;});var{data:players}=await window.supabase.from('players').select('id,current_username,status,banned_until,suspended_until,suspension_reason').in('id',playerIds);var pm={};(players||[]).forEach(function(p){pm[p.id]=p;});var{data:regs}=await window.supabase.from('match_registrations').select('player_id').eq('match_id',matchId);var regIds={};(regs||[]).forEach(function(r){regIds[r.player_id]=true;});
// Desempate determinista (motor compartido): el SQL ordena por imported_at
// (orden de importacion, arbitrario para ranking); aqui se fija el criterio
// 1) KD del partido 2) mas kills 3) menos muertes 4) alfabetico.
lastResultsState={res:res,pm:pm,regIds:regIds,showActions:canEditResults()};renderResultsTable();}catch(e){document.getElementById('results-list').innerHTML='<div class="text-center py-4 text-red-400 text-sm">Error: '+e.message+'</div>';}}
...
function openEditModal(){if(!currentMatch)return;document.getElementById('em-name').value=currentMatch.name||'';document.getElementById('em-game-id').value=currentMatch.game_id||'';document.getElementById('em-password').value=currentMatch.password||'';document.getElementById('em-type').value=currentMatch.match_type||currentMatch.type||'internal';document.getElementById('em-max').value=currentMatch.max_players||'';document.getElementById('em-desc').value=currentMatch.description||'';var emWa=document.getElementById('em-whatsapp');if(emWa)emWa.value=currentMatch.whatsapp_url||'';loadAllianceSelect('em-alliance',currentMatch.alliance_id);document.getElementById('edit-modal').classList.add('active');}
function closeEditModal(){document.getElementById('edit-modal').classList.remove('active');}
document.getElementById('edit-form').addEventListener('submit',async function(e){e.preventDefault();try{var{error}=await window.supabase.from('matches').update({name:document.getElementById('em-name').value,game_id:document.getElementById('em-game-id').value||null,password:document.getElementById('em-password').value||null,alliance_id:document.getElementById('em-alliance').value||null,match_type:document.getElementById('em-type').value,max_players:parseInt(document.getElementById('em-max').value)||null,description:document.getElementById('em-desc').value||null,whatsapp_url:(document.getElementById('em-whatsapp')?document.getElementById('em-whatsapp').value.trim()||null:null)}).eq('id',matchId);if(error)throw error;window.showToast('Actualizada','success');closeEditModal();loadMatch();}catch(e){window.showToast('Error: '+e.message,'error');}});
