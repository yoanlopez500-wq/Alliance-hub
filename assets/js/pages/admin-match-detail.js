...
async function deleteResult(resultId,playerId){
    if(!isSuperadmin()){window.showToast('Solo el superadmin puede eliminar resultados','error');return;}
...
async function openWinnersModal(){try{var{data:results}=await window.supabase.from('match_results').select('*').eq('match_id',matchId).order('kills',{ascending:false});var list=document.getElementById('winners-list');if(!results||results.length===0){list.innerHTML='<p class="text-sm text-center text-slate-400">Sin resultados</p>';}else{var playerIds=results.map(function(r){return r.player_id;}).filter(function(v,i,a){return a.indexOf(v)===i;});var{data:players}=await window.supabase.from('players').select('id,current_username').in('id',playerIds);var pm={};(players||[]).forEach(function(p){pm[p.id]=p;});list.innerHTML=results.map(function(r,i){var p=pm[r.player_id]||{};return'<div class="flex items-center gap-3 p-3 rounded-lg bg-slate-950 border border-indigo-900"><span class="text-lg font-bold w-6 text-amber-400">#'+(i+1)+'</span><div class="flex-1"><p class="font-medium text-sm text-slate-100">'+(p.current_username||'Jugador '+r.player_id)+'</p><p class="text-xs text-slate-400">'+r.kills+' bajas / '+r.deaths+' muertes</p></div><input type="checkbox" name="winner" value="'+r.player_id+'" class="w-5 h-5 rounded"></div>';}).join('');}document.getElementById('winners-modal').classList.add('active');}catch(e){window.showToast('Error: '+e.message,'error');}}
