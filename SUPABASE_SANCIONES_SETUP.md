...
- **Error `404` al hacer POST aunque la tabla existe**: casi siempre es porque faltan los `GRANT` de tabla a `anon`/`authenticated`. Las políticas RLS restringen filas, pero el rol necesita el privilegio de tabla primero. Solución:
  ```sql
  GRANT ALL ON public.player_strikes TO anon, authenticated;
  GRANT ALL ON public.strike_types TO anon, authenticated;
  GRANT ALL ON public.player_reports TO anon, authenticated;
  GRANT ALL ON public.player_sanctions TO anon, authenticated;
  ```
